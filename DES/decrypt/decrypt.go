...
func toBytes(array string) []byte {
	var result []byte
...
func keyRounds(L0, R0, key string) []string {
	...
	for round := 1; round <= 16; round++ {
		...
		previousC = currentC
		previousD = currentD
		keyrounds = append(keyrounds, K_i)
	}
	return keyrounds

}
func encrypt(L0 string, R0 string, key string) []byte {
	...
	cryptotext := toBytes(invPermuted)
	//fmt.Println(base64.StdEncoding.EncodeToString(cryptotext))
	return cryptotext

}
func mainEncrypt(message []byte, key []byte) []byte {

	keyStringOfBits := toBits(key)
...
func decrypt(L0 string, R0 string, key string) []byte {
	...
	keyrounds := keyRounds(L0, R0, key)

	for round := 16; round >= 1; round-- {
		currentL = previousR
		K_i := keyrounds[round-1]
		currentR = XOR(f(previousR, K_i), previousL)
		previousL = currentL
		previousR = currentR
	}
	...
	plaintext := toBytes(invPermuted)
	//fmt.Println(base64.StdEncoding.EncodeToString(cryptotext))
	return plaintext

}

func mainDecryption(cryptotext []byte, key []byte) []byte {
	...
	stringOfBits := toBits(cryptotext)
	...
	plaintext := decrypt(L0, R0, keyStringOfBits)
	return plaintext
}

func des2encryption(message []byte) []byte {

	key1 := []byte("cccccccc")
	key2 := []byte("gggggggg")
	cryptotext1 := mainEncrypt(message, key1)
	finalCryptotext := mainEncrypt(cryptotext1, key2)
	return finalCryptotext
...
func generateKeys() []string {
	...
	for y := 0; y < 256; y++ {
		...
		key := string(y) + string(y) + string(y) + string(y) + string(y) + string(y) + string(y) + string(y)
...
func allPossibleEncryptionK1(k1 []string, plaintext []byte) {

	myMap := make(map[string][]byte)
	for _, key := range k1 {
		y := mainEncrypt(plaintext, []byte(key))
		myMap[key] = y
	}

	for key, value := range myMap {
		L = append(L, Encryption{Key: key, Value: value})
	}

	sort.Slice(L, func(i, j int) bool {
		return bytes.Compare(L[i].Value, L[j].Value) < 0
	})

}

func findInList(valueToFind []byte) (bool, string) {

	for _, element := range L {
		if bytes.Equal(element.Value, valueToFind) {
			return true, element.Key
		}
	}
	return false, ""
}
...
var possiblePairsOfKeys []Keys

func allPossibleDecryptionK2(k2 []string, cryptotext []byte) {

	for _, key2 := range k2 {
		y := mainDecryption(cryptotext, []byte(key2))
		found, key1 := findInList(y)
		if found {
			// return key1, key2
			possiblePairsOfKeys = append(possiblePairsOfKeys, Keys{Key1: key1, Key2: key2})
		}
	}
}
func main() {

	plaintext := []byte("helloooo")
	// key := []byte("cccccccc")
	cryptotext := des2encryption(plaintext)
	// cryptotext := mainEncrypt(plaintext, key)
	fmt.Println("Plaintext:          ", plaintext)
	fmt.Println("Cryptotext:         ", cryptotext)

	// original := mainDecryption(cryptotext, key1)
	// fmt.Println("Original:           ", original)
	K1 := generateKeys()
	K2 := generateKeys()

	allPossibleEncryptionK1(K1, plaintext)
	allPossibleDecryptionK2(K2, cryptotext)

	if len(possiblePairsOfKeys) != 0 {
		fmt.Println("Chei gasite: ")
		for _, pair := range possiblePairsOfKeys {

			fmt.Println("K1:    ", pair.Key1)
			fmt.Println("K2:    ", pair.Key2)
			fmt.Println("-------------------")
		}

	} else {
		fmt.Println("Nu s-au gasit chei")
	}
